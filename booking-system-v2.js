                ...
                <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
                    <div class="flex items-center gap-2">
                        ...
                        <p class="text-sm text-purple-800">
                            <strong>Returning customers save 10%!</strong> We'll check your eligibility when you enter your email.
                        </p>
                    </div>
                </div>
            </div>
...
function showEmailCollectionStep() {
    ...
    const hasPromoCode = appliedPromoCode && appliedPromoCode.code;
            ...
            <div class="bg-gray-50 p-6 rounded-lg mb-6">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between text-green-600">
                        ...
                        <span class="font-semibold">${appliedPromoCode.code}</span>
                    </div>
                    ` : ''}
...
async function handleEmailSubmission(event) {
    ...
    await checkReturningCustomerStatus(customerEmail);
...
async function checkReturningCustomerStatus(email) {
    try {
...
// Show notification for returning customers
function showReturningCustomerNotification(purchaseCount) {
    // Create notification element if it doesn't exist
    const existingNotification = document.getElementById('loyalty-notification');
    if (existingNotification) {
        existingNotification.remove();
    }
    
    const notification = document.createElement('div');
    notification.id = 'loyalty-notification';
    notification.className = 'fixed top-4 right-4 bg-green-600 text-white px-6 py-4 rounded-lg shadow-lg z-50 animate-fade-in max-w-sm';
    notification.innerHTML = `
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <div class="text-sm opacity-90">10% loyalty discount applied to your package!</div>
            </div>
        </div>
    `;
...
async function setupPaymentForm() {
    ...
    // Display payment summary with email and discount info
    document.getElementById('payment-summary').innerHTML = `
        <div class="bg-gray-50 p-6 rounded-lg mb-6">
            ${isReturningCustomer && loyaltyDiscountApplied ? `
            ...
            <div class="space-y-2">
                ...
                ${loyaltyDiscountApplied && selectedPackage.originalPrice ? `
                <div class="border-t pt-2 mt-2">
                    <div class="flex justify-between text-gray-500">
                        <span>Original Price:</span>
                        <span class="line-through">$${selectedPackage.originalPrice}</span>
                    </div>
                ...
                <div class="border-t pt-2 mt-2">
                    <div class="flex justify-between text-lg font-bold">
                        <span>Total:</span>
                        <span class="text-green-600">${loyaltyDiscountApplied ? '$' + selectedPackage.price : '$' + selectedPackage.price}</span>
                    </div>
                    ${loyaltyDiscountApplied ? `
                    <div class="text-xs text-green-600 text-right mt-1">
                        You saved $${selectedPackage.originalPrice - selectedPackage.price}!
                    </div>
                    ` : ''}
                </div>
            </div>
        ...
        try {
            const response = await fetch('/.netlify/functions/create-payment', {
                ...
                body: JSON.stringify({
                    ...
                    originalPrice: selectedPackage.originalPrice ? selectedPackage.originalPrice * 100 : null
                })
            });
